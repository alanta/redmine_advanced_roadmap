...
xml.issue do
  ...
  xml.milestones do
    @issue.fixed_version.milestones.each do |milestone|
      xml.milestone(:id => milestone.id, :name => milestone.name)
    end
  end unless @issue.fixed_version.nil? or @issue.fixed_version.milestones.empty?
